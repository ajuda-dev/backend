...
```
backend/
├── .env.exemple          # environment variables template (DB_*)
├── go.mod / go.sum       # Go 1.24.0 module
├── main.go               # entrypoint → config.InitApp()
...
| GET | `/v1/community` | `GetAllCommunities` | Paginated community listing; query params `page`, `limit`, `address_id` (200) |
| GET | `/swagger/*` | `swagger.HandlerDefault` | Swagger UI documentation |

## How to run

- **Run:** `go run main.go` (starts the Fiber server on port **8080**).
- **Build:** `go build`.
