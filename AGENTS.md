...
| PUT | `/v1/event/:id/reschedule` | `RescheduleEvent` | Who can manage the event (`canManageEvent`) changes `start_at` (must be in the future) and writes a required `comment` (trim, max 500) on `events.comment`. A later reschedule overwrites the same field. Approval status and participants are unchanged. 200 `EventDto` with `comment`. Third party → **403**. |
